<x-layouts.app :title="$title" activeTab="">
    ...
    >
        <div class="w-full max-w-[480px] bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-9 text-left">
            ...
            <div class="mb-5">
                <h1 class="text-xl sm:text-2xl font-black text-gray-900 tracking-tight" x-text="role === 'seller' ? 'Daftar Akun Calon Seller' : 'Buat Akun Pembeli'">
                </h1>
                <p class="text-xs sm:text-[13.5px] text-gray-500 mt-1 leading-relaxed" x-text="role === 'seller' ? 'Daftarkan akunmu terlebih dahulu untuk mulai proses aktivasi seller resmi di WhiMarket.' : 'Mulai belanja barang pre-loved & merchandise langsung dari kreator idolamu.'">
                </p>
            </div>

            <!-- PRD Info Box for Calon Seller -->
            <template x-if="role === 'seller'">
                <div class="p-3.5 rounded-2xl bg-[#F3EEFF]/60 border border-purple-100 text-left mb-5">
                    <div class="flex items-center gap-1.5 text-xs font-bold text-[#4F26A6] mb-1.5">
                        <svg class="w-4 h-4 text-[#4F26A6] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span>Alur Pendaftaran Seller WhiMarket:</span>
                    </div>
                    <ol class="text-[11.5px] text-gray-600 space-y-1 list-decimal list-inside leading-relaxed">
                        <li>Daftarkan akun menggunakan formulir di bawah ini.</li>
                        <li>Hubungi Admin via DM TikTok / Instagram <strong>@whimarket</strong> untuk verifikasi email.</li>
                        <li>Dapatkan Kode Akses VIP resmi lalu aktifkan toko di menu seller.</li>
                    </ol>
                </div>
            </template>
            ...
            <form id="register-form" action="{{ route('register.submit') }}" method="POST" class="space-y-4">
                ...
                >
                    <span x-text="role === 'seller' ? 'Daftar Akun Seller' : 'Daftar Akun'"></span>
            ...
            <div class="pt-5 mt-5 border-t border-gray-100 text-center">
                <p class="text-xs text-gray-500 font-medium">
                    Sudah punya akun WhiMarket?
                    <a :href="'{{ route('login') }}?role=' + role" class="text-[#4F26A6] font-bold hover:underline ml-1">
                        Masuk Sekarang
                    </a>
                </p>
            </div>
